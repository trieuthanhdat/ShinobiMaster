...
namespace TD.GlobalTimer
{
    ...
    public class GlobalTimerCounter : MonoSingleton<GlobalTimerCounter>, ITimeProvider
    {
        [SerializeField] DateTimeFormatType dateTimeFormat = DateTimeFormatType.Hour_Minutes_Seconds;
        [SerializeField] GetServerTimeMethod getServerTimeMethod = GetServerTimeMethod.PlayfabApi;
        ...
        public void OnNewDayComeTriggered()
        {
            OnNewDayCome?.Invoke();
        ...
        public void OnNewWeekComeTriggered()
        {
        ...
        private void ResetTimer(bool hardReset = true)
        {
            ...
            switch(getServerTimeMethod)
            {
                case GetServerTimeMethod.PlayfabApi:
                    OnPlayfabAPIResetTime();
                    break;
                default:
                    OnCustomAPIResetTime(hardReset);
                    break;
            }

        }
        private void OnPlayfabAPIResetTime()
        {
            m_IsReseting = true;
            GetPlayfabServerTime(() =>
            {
                m_IsReseting = false;
            });
        }
        private void OnCustomAPIResetTime(bool hardReset)
        {
        ...
        public void GetTimeServer()
        {
            switch (getServerTimeMethod)
            {
                case GetServerTimeMethod.PlayfabApi:
                    ...
                    break;
                default:
                case GetServerTimeMethod.CustomApi:
        ...
        {
            DateTime onlineDate = UnixTimeStampToDateTime(serverTimeStamp);
            Debug.Log("GLOBAL TIMER: Server time = " + onlineDate.ToString());
            _HasGotServerTime = true;
            DateTime nextDay = TryGetSaveDate(out DateTime saveDay) ? saveDay.AddDays(1) : onlineDate.AddDays(1);
            nextDay = nextDay.Date; // Ensure next day starts at midnight (00:00:00)
            Debug.Log("GLOBAL TIMER: Next reset time = " + nextDay.ToString());
            UpdateNextTimeReset(nextDay);
            ParseDateTime(onlineDate);
            Refresh();
            _isInited = true;
        }
        ...
        {
#if ENABLE_PLAYFAB
            CloudScripts.getPlayfabServerTime((responseData) =>
            {
                if (responseData.IsSuccess)
                {
                    SetupServerTime(responseData.serverTimestamp);
                    onSuccess?.Invoke();
                }
                else
                {
                    Debug.Log("GLOBAL TIMER: Server time error - " + responseData.errorMsg);
                    HandleServerTimeError(onSuccess, onError);
                    return;
                }
            }, (error) =>
            {
                Debug.Log("GLOBAL TIMER: Failed to get server time => use device time. Error: " + error);
                onError?.Invoke(error.HttpCode);
            });
#endif
        }


        private void HandleServerTimeError(Action onSuccess = null, Action<int> onError = null)
        {
            _retryTime++;
            if (_retryTime < _retryTimeAllowed)
            {
                GetPlayfabServerTime(onSuccess, onError);
            }
            else
            {
                _isInited = false;
        ...
        private IEnumerator Co_GetServerTimeWebRequest()
        {
            using (var webRequest = UnityWebRequest.Get(SERVER_TIME_URL))
            {
                ...
                {
                    Debug.LogError("GLOBAL TIMER: Failed to retrieve server time: " + webRequest.error);
        ...
        private void StoreDateAndIndex()
        {
            ...
            Debug.Log($"GLOBAL TIMER: store date: {m_CurrentDate} - day index: {m_CurrentDayIndex} - week index: {m_CurrentWeekIndex} - is new day "+_isNewDay);
        }
        ...
        private void AdvanceNewDay()
        {
            int savedDayIndex = PlayerPrefs.GetInt(SYSTEM_DAY_INDEX_STRING_KEY, 0);
            m_CurrentDayIndex += 1;
            StoreDateAndIndex();
        }
        private void AdvanceNewWeek()
        {
            int savedDayIndex = PlayerPrefs.GetInt(SYSTEM_WEEK_INDEX_STRING_KEY, 0);
            m_CurrentDayIndex += 1;
            StoreDateAndIndex();
        }
        #endregion
        ...
        {
            if(Instance != null && Instance is ITimeProvider)
                return (ITimeProvider)Instance;
        ...
        public void ValidateDateTimeFormat(ref DateTimeFormatType currentFormat)
        {
            ...
            else if (timeUntilNextDay.TotalMinutes < 1 && timeUntilNextDay.TotalHours <= 0)
            {
...
public enum GetServerTimeMethod
{
    None,
    PlayfabApi,
    CustomApi
