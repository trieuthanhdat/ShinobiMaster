...
public class IpadContentSettings : MonoBehaviour
{
    ...
    {
        _isIpad = RectTransformUtils.IsIpadOrTablet();
        if (!_isIpad) return;
    ...
    private void ApplyAnchorSettings()
    {
        if (setNewAnchor)
        {
            RectTransformUtils.SetAnchor(contentTransform, anchorPreset);
        }
    }
    ...
    private void ApplyPivotSettings()
    {
        if (setNewPivot)
        {
            RectTransformUtils.SetPivot(contentTransform, pivotPreset);
        }
    }
    ...
    private void ApplyPositionSettings()
    {
        if (setNewPosition)
        {
            ...
            if (!setNewAnchor)
            {
                ...
                AnchorPresets currentAnchorPreset = RectTransformUtils.GetCurrentAnchorPreset(contentTransform.anchorMin, contentTransform.anchorMax);
